...
def plot_save_slice_centroids(slm, savepath, h5plot=True, time=None, zeta_pos=None, t_is_z=True):
    ...
    Xb0_norm = np.ones(slm.avgx2[0,:].shape)
    Yb0_norm = np.ones(slm.avgx3[0,:].shape)
    for i in range(0,len(slm.zeta_array)):
        if (slm.avgx2[0,i] != 0.0):
            Xb0_norm[i] = slm.avgx2[0,i]
            Yb0_norm[i] = slm.avgx3[0,i]
...
#        if (slm.zeta_array[i] <= zeta_hseed):
        Xb_norm[:,i] = np.absolute( slm.avgx2[:,i]/Xb0_norm[i] )
        Yb_norm[:,i] = np.absolute( slm.avgx3[:,i]/Yb0_norm[i] )
