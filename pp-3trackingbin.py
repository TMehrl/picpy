...
def binSlab_parser():
    ...
    parser.add_argument(  "--ion_length",
                          dest = "ion_length",
                          action="store_true",
                          default=False,
                          help = "Enables the ionization length (Default: %(default)s).")
...
def calc_analytical_solution(start_r_array, nb, ni, lbunch, zeta_end, rbunch, ion_length_input):
        ...
        for k in range(len(w)):
            ...
            for i in range(len(d)):
                ...
                for j in range(int(np.floor(len(e)/modnum))):
                    ...
                    starting_positions.append(x[-1]) #799])#
            ...
            #input_r_array = np.array([8.326672684688674e-17, 0.0470588244497776, 0.0941176488995552, 0.1411764770746231, 0.1882352977991104, 0.23529411852359772, 0.2823529541492462]) #,0.0352])
            #input_r_array = np.array([ 0.07, 0.14076246321201324, 0.21])
            #input_r_array = np.array([ 0.5, 1, 1.5, 1.8])
            #input_r_array = np.array(starting_positions)
            ...
            for i in range(len(input_r_array)):
                zeta_array3, r_matrix2 = calc_analytical_solution(input_r_array[i], 1.2, 1, 2, 12, 0.3, args.ion_length) #36, 0.3)#8,2)  ###(start_r_array, nb, ni, lbunch, zeta_end, rbunch):
                #ax.plot(zeta_array3[np.where(r_matrix2 >0)], r_matrix2[np.where(r_matrix2 >0)], color = 'black',linestyle = '-.' ) ##00cc00
                ax.plot(zeta_array3, r_matrix2, color = 'magenta', linewidth=1.2*args.linewidth, linestyle= '-.') ##00cc00
            ...
            if args.track_color == "u_tot":
                cbar.ax.set_title(r'$|u|$', fontsize=16)
            elif args.track_color == "beta_z":
                cbar.ax.set_title(r'$\beta_z$', fontsize=16)
            elif args.track_color == "beta_y":
                cbar.ax.set_title(r'$\beta_y$', fontsize=16)
            ...
            analytical_solutions = mlines.Line2D([], [], color='magenta', label='analytical model', linestyle='-.')
            ax.legend(handles=[analytical_solutions ], fontsize=16)
